...
def _reglas_de_ejercicio(datos: dict, contenido: dict, etapa: str, r: Resultado) -> None:
    ...
    if datos.get("tipo") == "emparejar":
        ...
        opciones = (contenido.get("opciones") or [])[: int(contenido["porRonda"])]
    else:
    ...
    if tipo == "emparejar":
        ...
        sin_audio = [
            ...
            # Suena por muestra suelta, por nota transportada o por ritmo con el clic:
            # las tres valen, la regla es que suene.
            if not e.get("audio") and not e.get("nota") and not e.get("ritmo")
        ]
        if sin_audio:
            r.avisos.append(
