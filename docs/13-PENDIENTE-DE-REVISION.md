...
"$comment": "PENDIENTE DE REVISIÓN PEDAGÓGICA: (1) la melodía es «Ah! vous dirai-je, maman», anónima del siglo XVIII y en dominio público; la letra en español —«Estrellita, ¿dónde estás?»— es la adaptación tradicional que circula como anónima, pero hay que confirmar su origen antes de dar por buena la primera estrofa (CLAUDE.md §10: todo repertorio se verifica contra una fuente de dominio público). (2) El reparto de zonas —palmas en las sílabas del arranque, muslos en las del medio y pies en la nota larga; chasquidos para abrir las frases del medio— es una propuesta de percusión corporal, no un arreglo canónico. (3) La melodía escrita es la de «Ah! vous dirai-je, maman» en do mayor, la de siempre; suena bajita con la flauta debajo de los golpes.", "consigna": "actividad.c136.consigna", "tempo": 92, "patron": [

## 3. `content/actividades/c1-37-debajo-un-boton-con-el-cuerpo.json` (línea 26)

"$comment": "PENDIENTE DE REVISIÓN PEDAGÓGICA: (1) «Debajo un botón» es una canción tradicional española anónima, y así consta en los cancioneros escolares; conviene confirmarlo contra una fuente de dominio público antes de darla por buena. Se eligió en lugar de «La vaca lechera», que el autor pidió y que NO es tradicional: música de Fernando García Morcillo (†2002) y letra de Jacobo Morcillo, protegida hasta bien entrada la década de 2070 (CLAUDE.md §10). (2) El ritmo escrito —cuatro corcheas, una negra y las dos negras del eco en cada frase— es la lectura habitual del estribillo; una maestra debe confirmar que cuadra con cómo se canta en su clase. (3) El reparto de zonas es una propuesta. (4) Sin melodía todavía: no se ha escrito de memoria a propósito —una melodía mal transcrita enseña algo falso—; en cuanto haya una transcripción verificada, va en `melodia`, una nota por golpe, y sonará bajita como en «Estrellita».", "consigna": "actividad.c137.consigna", "tempo": 96, "patron": [

## 4. `content/actividades/c2-23-termina-o-se-queda-a-medias.json` (línea 25)
...
## 5. `content/actividades/c2-24-familias-de-instrumentos.json` (línea 25)
...
## 6. `content/actividades/c2-28-vals-marcha-o-tango.json` (línea 25)
...
## 7. `content/actividades/c2-31-alegre-o-triste.json` (línea 25)
...
## 8. `content/actividades/c2-36-que-estilo-suena.json` (línea 25)
...
## 9. `content/actividades/c2-37-quien-canta.json` (línea 25)
...
## 10. `content/actividades/c3-18-escala-mayor-o-menor.json` (línea 25)
...
## 11. `content/actividades/c3-20-que-compas-es.json` (línea 26)
...
## 12. `content/actividades/c3-24-quien-compuso-esta-melodia.json` (línea 25)

"consigna": "actividad.c324.consigna", "$comment": "PENDIENTE DE REVISIÓN PEDAGÓGICA: las siete melodías están transcritas de memoria y simplificadas a una sola voz y a las figuras del ciclo; hay que confirmarlas frente a una partitura de dominio público antes de darlas por buenas.", "opciones": [

## 13. `content/actividades/c3-25-el-compas-de-doce.json` (línea 27)
...
## 14. `content/actividades/c3-26-bailes-y-compases.json` (línea 25)
...
## 15. `content/actividades/c3-31-ritmos-del-mundo.json` (línea 25)
...
## 16. `content/actividades/c3-34-coloca-la-nota-en-clave-de-fa.json` (línea 25)
...
## 17. `content/actividades/c3-35-suena-bien-o-roza.json` (línea 25)
...
## 18. `content/actividades/c3-38-ocho-estilos.json` (línea 25)
...
## 19. `content/criterios.json` (línea 2)
...
## 21. `src/motor/afinacion.ts` (línea 67)
...
## 25. `src/motor/rejillaRitmica.ts` (línea 9)
...
## 26. `src/motor/repaso.ts` (línea 18)
...
## 27. `tools/auditoria.mjs` (línea 271)
...
## 28. `tools/dificultad.mjs` (línea 73)
...
## 29. `tools/dificultad.mjs` (línea 251)
...
## 30. `tools/muestras-provisionales.py` (línea 98)
...
## 31. `tools/muestras-provisionales.py` (línea 144)
