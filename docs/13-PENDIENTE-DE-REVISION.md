...
"consigna": "actividad.c236.consigna", "$comment": "PENDIENTE DE REVISIÓN PEDAGÓGICA: los fragmentos de rock, vals y salsa son piezas de Kevin MacLeod (CC BY 3.0) compuestas «en el estilo de», y el rap es una base instrumental CC0. Son ejemplos limpios de manual; no son grabaciones históricas de cada estilo.", "opciones": [
...
## 51. `content/criterios.json` (línea 2)
...
## 53. `src/motor/afinacion.ts` (línea 67)
...
## 55. `src/motor/evaluacion.ts` (línea 197)
...
## 56. `src/motor/musicograma.ts` (línea 32)
...
## 57. `src/motor/pautaRitmica.ts` (línea 14)
...
## 58. `src/motor/rejillaRitmica.ts` (línea 9)
...
## 59. `src/motor/repaso.ts` (línea 18)
...
## 60. `tools/auditoria.mjs` (línea 271)
...
## 61. `tools/dificultad.mjs` (línea 73)
...
## 62. `tools/dificultad.mjs` (línea 251)
...
## 63. `tools/muestras-provisionales.py` (línea 98)
...
## 64. `tools/muestras-provisionales.py` (línea 144)
